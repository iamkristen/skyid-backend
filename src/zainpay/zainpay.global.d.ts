interface ZainpayArgs {
  publicKey: string;
  serviceType: ServiceType[keyof ServiceType];
  sandbox: boolean;
  data?: Record<string, unknown>;
  params?: string;
}

declare module "zainpay-nodejs-sdk" {
  export function Zainpay(args: ZainpayArgs): Promise<any>;
  export const serviceTypes: ServiceType;

  export interface ZainpayInitializePaymentResponse {
    code: string;
    data: string;
    description: string;
    status: string;
  ...
  export interface ZainpayFundsTransferResponse {
    code: string;
    description: string;
    status: string;
  }

  export interface ZainpayBankList {
    code: string;
